...
public class KafkaStreamTopology {
    ...
    public void process(StreamsBuilder streamsBuilder){
      var jsonSerde = new JsonSerde<>(Order.class);
        var streams  = streamsBuilder
            .stream(SOURCE,
                Consumed.with(Serdes.String(), jsonSerde));

        var modifiedStream = streams
            .mapValues((key, value)-> {
              value.getAddress().setCity(value.getAddress().getCity().toUpperCase());
              return value;
            });
            modifiedStream.to(DESTINATION_OR_SINK, Produced.with(Serdes.String(),
                                                                 jsonSerde));
      var modifiedStream1 = streams
          .mapValues((key, value)-> {
            value.getAddress().setCity("Pandora");
            return value;
          });
      modifiedStream1.to(DESTINATION_OR_SINK, Produced.with(Serdes.String(),
                                                           jsonSerde));
    }
